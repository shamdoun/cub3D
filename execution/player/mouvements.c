...
/*   Updated: 2024/10/20 13:10:03 by shamdoun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_up(t_player *p, char **mapValues)
{
	...
	old_x = p->x_p;
	old_y = p->y_p;
	p->x_p = p->x_p + cos(-p->angle * (M_PI / 180)) * 9;
	p->y_p = p->y_p + sin(-p->angle * (M_PI / 180)) * 9;
...
void	move_right(t_player *p, char **mapValues)
{
	...
	old_x = p->x_p;
	old_y = p->y_p;
	p->x_p = p->x_p + cos(-p->angle * (M_PI / 180) + (M_PI / 2)) * 9;
	p->y_p = p->y_p + sin(-p->angle * (M_PI / 180) + (M_PI / 2)) * 9;
	if (wall_contact(p, mapValues))
	{
		p->x_p = old_x;
		p->y_p = old_y;
